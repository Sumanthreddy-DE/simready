...
def test_list_demo_steps_finds_degraded_fixtures() -> None:
    ...
    if not any(Path("data/parametric_degraded").glob("*.step")):
        pytest.skip("data/parametric_degraded is gitignored local data "
                    "(regenerate via scripts/generate_degraded_steps.py)")
...
def test_grouped_demo_steps_returns_source_labels() -> None:
    ...
    if not grouped:
        pytest.skip("no demo STEPs on disk — data/parametric_degraded and "
                    "tests/data/grabcad are both gitignored local data")
