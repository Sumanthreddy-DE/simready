...
- `simready.pipeline.analyze_file_safe`: killable child via `python -m simready.pipeline_worker` (plain `subprocess.Popen` + `kill()`, JSON handoff through a temp file) with the standard `AnalysisTimeout` report; wired into the copilot `analyze_geometry` tool, both Streamlit entry points, and the CLI. The in-process `analyze_file` remains for batch callers that manage their own guards.
- **Why not `multiprocessing`:** the first implementation used `mp.spawn` — it re-executes the parent `__main__` module in the child, which crashes under Streamlit (the app script *is* `__main__`, and the analyze call sits at module top level), and its bootstrap added enough overhead that the two slow-but-healthy parts still tripped a 90 s budget. The subprocess worker completes them in 8.5 s / 11.8 s wall.
- Regression: `tests/test_real_eval_regression.py` (skip-if-absent — data is gitignored) pins all 4 former kill parts to full completion in ≤ 90 s plus the flange `SelfIntersectionSkipped` finding.
