...
- **defect-head-augmentation-attempt** (S2 partial — item stays Open w/ findings) — built combined-v2 (2272 graphs: +293 featured cleans via new `--fillet-prob/--chamfer-prob` generator flags, +879 featured degraded, `_f` stems after a label-collision detour), retrained 30 epochs source-grouped: val defect acc 0.686 (harder val), open_shell 0.571→0.856, refinement fixtures acc/recall 0.853/0.895 (v1: 0.580/0.527 same fixtures). **Real-CAD FP rate unchanged: 11/11** (coverage 7→11 via hang fixes). Honest negative recorded; synthetic augmentation on box/cyl grammar declared exhausted. Tracked weights/metrics refreshed. `docs/validation/defect_classifier.md` v2 + `real_eval_v2.md`. *(SHA: this commit)*
