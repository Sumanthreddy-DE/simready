...
- **adr-backlog** (S3) — ADRs 0002 (OpenAI-compat SDK over Anthropic-native, base_url portability + provider-quirk ledger), 0003 (RAG-lite JSON+cosine over vector DB, sizing judgment), 0004 (stateless agent + `AgentResponse.messages` round-trip, one history representation for UI/CLI/persistence/traces). Retro-documented with original decision dates. *(SHA: this session's docs commit.)*
